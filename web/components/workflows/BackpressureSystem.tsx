...
export function BackpressureSystem() {
  ...
  const [submitting, setSubmitting] = useState<'single' | 'burst' | 'worker'>();
    ...
    <main className="min-h-screen bg-background text-foreground">
      ...
      <div className="mx-auto max-w-[110rem] space-y-5 p-3 sm:p-5 lg:p-6">
        <section className="grid gap-4 rounded-2xl border bg-card p-5 shadow-sm lg:grid-cols-[1fr_auto] lg:items-center lg:p-6">
          <div>
            <h1 className="max-w-4xl font-serif text-2xl font-semibold tracking-tight sm:text-3xl">
              Durable execution playground
            </h1>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">
              Create a burst, add local Workers, and watch isolated agents drain the queue live.
            </p>
          </div>
          <div className="flex flex-wrap items-end gap-2 lg:justify-end">
            ...
            <Button
              ...
              disabled={
                submitting !== undefined || !snapshot ||
                snapshot.worker.configuredJobConcurrency >= snapshot.worker.maxJobWorkerCapacity
              }
            >
              {submitting === 'worker' ? <LoaderCircleIcon className="animate-spin" /> : <UserPlusIcon />}
              Add Worker
            </Button>
          </div>
        </section>
  ...
  const metrics = [
    {
      ...
      detail: `${counts.waiting} blocked, oldest eligible ${counts.oldestQueuedSeconds}s`,
      icon: GaugeIcon,
    ...
    },
    {
      ...
      detail: `${formatLatency(snapshot.latency.executionP50Ms)} p50 execution`,
      icon: BoxesIcon,
    },
    {
      label: 'Notification backlog',
      value: String(counts.notificationsQueued + counts.notificationsDelivering),
      detail: `${formatLatency(snapshot.latency.notificationDeliveryP50Ms)} p50 delivery`,
      icon: ActivityIcon,
    },
    {
      label: 'p50 end to end',
      value: formatLatency(snapshot.latency.endToEndP50Ms),
...
function ActivityTrace({
  ...
  return (
    <aside className="flex min-h-[31rem] flex-col overflow-hidden rounded-2xl border bg-card text-foreground shadow-sm">
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <span className={cn(
            ...
            <span className={cn(
              ...
              live ? 'animate-pulse bg-emerald-500 motion-reduce:animate-none' : 'bg-amber-500',
            )} />
      ...
      <div className="scrollbar-thin min-h-0 flex-1 overflow-y-auto px-4 py-3">
        {activity.length === 0 ? (
          <div className="grid min-h-72 place-items-center text-center text-xs leading-5 text-muted-foreground">
          ...
          <ol className="relative ml-1 border-l pl-4">
            {activity.slice(0, 30).map((item) => (
              <li key={item.id} className="relative pb-4">
                <span className={cn(
                  'absolute -left-[1.17rem] top-1 size-2 rounded-full ring-4 ring-white',
                  item.source === 'notification' ? 'bg-violet-500' : 'bg-blue-500',
                )} />
                <div className="flex items-start justify-between gap-2">
                  <code className="break-all text-[0.66rem] font-semibold text-foreground">
                    {item.type}
                  </code>
                  <time className="shrink-0 font-mono text-[0.55rem] text-muted-foreground">
