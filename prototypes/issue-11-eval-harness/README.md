...
1. **Paired journey:** run the same synthetic broker requests through the
   ...
   Authorization, bounded responses, correct resolution, and no unsafe mutation
   are correctness gates. Packet reads, context bytes, approximate tokens, and
   tool counts are diagnostics because smaller context is useful only while it
   stays sufficient. The current one-Packet-per-turn behavior is a V0 policy to
   evaluate, not a timeless architecture invariant.
