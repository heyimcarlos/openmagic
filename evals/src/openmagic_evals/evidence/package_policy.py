...
from openmagic_evals.evidence.sql_scanner import sql_ownership_violations
...
PACKAGE_ROLES: tuple[PackageRole, ...] = (
    ...
    PackageRole(
        ...
        sql_owner_roots=(
            ...
            Path("evidence/_race_persistence.py"),
...
def role_sql_ownership_violations(role: PackageRole, paths: tuple[Path, ...]) -> tuple[str, ...]:
    ...
    return sql_ownership_violations(
        distribution=role.distribution,
        package=role.package,
        owner_roots=role.sql_owner_roots,
        paths=paths,
    )
